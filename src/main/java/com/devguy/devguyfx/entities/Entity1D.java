...
import com.devguy.devguyfx.entities.textrender.Text;
...
public class Entity1D implements IEntity {
    ...
    public void render(Entity1D[][] map, Point cursor) {
        ...
        muzzlePoints.clear();
        bodyPositions.clear();
        for (int map_x = cursor.x, ent_x = 0; ent_x < representMap[0].length && map_x < this.currentLevel.width; map_x++, ent_x++) {
            for (int map_y = cursor.y, ent_y = 0; ent_y < representMap.length && map_y < this.currentLevel.height; map_y++, ent_y++) {
